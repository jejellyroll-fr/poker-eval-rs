...
pub fn std_deck_omaha_hi_low8_eval(
    ...
    let eligible_for_low = allval != LowHandVal { value: LOW_HAND_VAL_NOTHING };
    ...
    let mut best_lo = LowHandVal { value: LOW_HAND_VAL_NOTHING }; // Assurez-vous que c'est la valeur appropriée pour "rien"
...
mod tests {
    ...
    fn test_omaha_high_hand_evaluation() {
        ...
        let (hole, _) = StdDeck::string_to_mask(hole_str).expect("Failed to convert hole string to mask");
        let (board, _) = StdDeck::string_to_mask(board_str).expect("Failed to convert board string to mask");
        ...
        assert_eq!(hival, Some(expected_high_value), "High hand value did not match expected value");
    }
    ...
    fn test_omaha_low_hand_evaluation() {
        ...
        let mut loval = None;
        std_deck_omaha_hi_low8_eval(hole, board, &mut None, &mut loval).expect("Low hand evaluation failed");

        let expected_low_value = LowHandVal { value: 274960 }; // Valeur représentant As2d4h3c5d

        assert_eq!(loval, Some(expected_low_value), "Low hand value did not match expected value");
    }
